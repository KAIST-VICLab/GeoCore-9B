...
def main(args):
    ...
    if args.base_ckpt and os.path.exists(args.base_ckpt):
        if accelerator.is_main_process:
            print(f"Loading pretrained weights from {args.base_ckpt}...")
        
        pretrain_dict = torch.load(args.base_ckpt, map_location='cpu', weights_only=False)
        
        if "model" in pretrain_dict:
            model_state_dict = pretrain_dict["model"]
        else:
            model_state_dict = pretrain_dict
            
        clean_state_dict = OrderedDict()
        for k, v in model_state_dict.items():
            name = k.replace('_orig_mod.', '')  # strip torch.compile prefix
            clean_state_dict[name] = v
            
        msg = model.load_state_dict(clean_state_dict, strict=False)
        if accelerator.is_main_process:
            print(f"Pretrained model loaded with message: {msg}")
            
        ema = deepcopy(model).to(device)
        if "ema" in pretrain_dict:
            ema_state_dict = pretrain_dict["ema"]
            clean_ema_dict = OrderedDict()
            for k, v in ema_state_dict.items():
                name = k.replace('_orig_mod.', '')
                clean_ema_dict[name] = v
            ema.load_state_dict(clean_ema_dict, strict=False)
            if accelerator.is_main_process:
                print("Pretrained EMA weights successfully applied.")
    else:
        if accelerator.is_main_process:
            print(f"Warning: No base checkpoint found at {args.base_ckpt}. Training from scratch.")
        ema = deepcopy(model).to(device)
    # -----------------------------------------------------------------
    ...
    if args.resume_step > 0:
        ...
        if os.path.exists(resume_path):
            ...
            if accelerator.is_main_process:
                logger.warning(f"Checkpoint folder {resume_path} not found!")
    ...
    for epoch in range(args.epochs):
        ...
        for x, cap, meta in train_dataloader:
            ...
            grad_norm_tensor = torch.tensor(grad_norm, device=device) if 'grad_norm' in locals() else torch.tensor(0.0, device=device)
            logs = {
...
def parse_args(input_args=None):
    ...
    parser.add_argument("--base-ckpt", type=str, required=True,
                        help="Pre-trained GeoCore-9B checkpoint to fine-tune from")
    ...
    parser.add_argument("--adam-epsilon", type=float, default=1e-15)
